...
def commands_home(request):
    return render(request, '../templates/django_kafka/commands/commands-home.html')

@user_passes_test(permission_roles.is_login)
def get_commands_from_last_offset(request):
    if request.method == "GET":
        username = request.user.username
        topic = "commands"
        data = read_from_topic(topic, username)
        return render(request,
                      '../templates/django_kafka/commands/commands-last-massage.html',
                      {'topic_data': data})


@user_passes_test(permission_roles.is_login)
def get_commands_from_last_n_Message(request):
    if request.method == "GET":
        username = request.user.username
        topic = "commands"
        ...
        return render(request, '../templates/django_kafka/commands/commands-last-massage.html',
                      {'topic_data': data})


@user_passes_test(permission_roles.is_login)
def get_commands_from_beginning(request):
    if request.method == "GET":
        username = request.user.username
        topic = "commands"
        data = read_from_topic(topic, username, None, True)
        return render(request, '../templates/django_kafka/commands/commands-from-beginng.html',
                      {'topic_data': data})


@user_passes_test(permission_roles.is_login)
def get_commands_between_timestamps(request):
    if request.method == "GET":
        username = request.user.username
        topic = "commands"
        from_timestamp = None
        ...
        if from_timestamp is None and to_timestamp is None:
            return render(request, '../templates/django_kafka/commands/commands-timestamps.html',
                          {'topic_data': []})
